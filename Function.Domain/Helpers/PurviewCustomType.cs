...
namespace Function.Domain.Helpers
{
    ...
    public class PurviewClient
    {
        ...
        private async Task<string> GetToken()
        {
            ...
            {
                return ((Azure.Core.AccessToken)_cache.Get("token")).Token;
            }
            ...
            try
            {
                var cred = new DefaultAzureCredential(); //uses managed identity of the function or of the developer
                var tokenctx = new Azure.Core.TokenRequestContext(scopes);
                token = cred.GetToken(tokenctx);
            }
            ...
            //_token = result;
            var cacheItem = new CacheItem("token", token);
